...
void simulateBashShell() {
    char * command, ** formatted_command;
    int args_counter = 0, is_linux_command = FALSE, stop = FALSE;
    ...
    do {
        ...
        // Get command introduced by the user and convert it to lower case
        command = readLineUntilDelimiter(STDIN_FILENO, '\n');
        command = toLowerCase(command);

        // Split command by spaces and get the number of arguments
        formatted_command = split(command, " ");
        args_counter = countSplits(formatted_command) - 1;

        // Check if command matches with a custom or Linux command
        if(strcmp(formatted_command[0], custom_commands[0]) == 0) {
        ...
        } else if(strcmp(formatted_command[0], custom_commands[1]) == 0) {
        ...
        } else if(strcmp(formatted_command[0], custom_commands[2]) == 0) {
        ...
        } else if(strcmp(formatted_command[0], custom_commands[3]) == 0) {
        ...
        } else if(strcmp(formatted_command[0], custom_commands[4]) == 0) {
            ...
            runLinuxCommand(formatted_command);
        }        
    } while(!stop);
}